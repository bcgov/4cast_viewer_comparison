...
-   Here we compare the current $C_t$ and previous Stokes forecasts $P_t$, using two metrics.

$${\displaystyle \operatorname {RMSE} ={\sqrt {{\frac {1}{n}}\sum _{t=1}^{n}(C_{t}-P_{t})^{2}}}}$$

-  Note that RMSE is a scale dependent metric: the same proportional difference (e.g. 10%) yields a larger RMSE for larger occupations.

$${\displaystyle {\text{SMAPE}}={\frac {1}{n}}\sum _{t=1}^{n}{\frac {|C_{t}-P_{t}|}{|C_{t}|+|P_{t}|}}}$$

-  In contrast, SMAPE is a scale independant metric: Note that the same absolute difference (e.g. 1000) yields a larger SMAPE for *smaller* occupations. 

- In the scatter plot to the right we can see that similar forecast bottom left, different forecasts top right.  Occupation size by size and colour. 
...
plotly::renderPlotly({
  ...
  plt <- reactive_tbbl()|>
    ggplot(aes(rmse, smape, size=mean_value, colour=mean_value, alpha=mean_value, text=slice))+
  geom_point()+
...
plotly::renderPlotly({
  ...
  plt <- prop_data|>
    ...
    ggplot(aes(year, prop, colour=series))+
    geom_line()+
    labs(x=NULL, y=NULL, colour=NULL)+
